...
func TestCSIVolumeCountPredicate(t *testing.T) {
	...
	for _, test := range tests {
			...
			pred := predicates.NewCSIMaxVolumeLimitPredicate(getFakeCSINodeLister(csiNode),
			...
			pred.NodeVolumeLimits(test.newPod, NodeVolumeLimits)
		})
...
func getNodeWithPodAndVolumeLimits(limitSource string, pods []*v1.Pod, limit int64, driverNames ...string) (*resourceinfo.NodeInfo, *storagev1.CSINode) {
	nodeInfo := resourceinfo.NewNodeInfo(pods...)
	node := &v1.Node{
