...
var (
	...
	// apiHost = "10.96.0.1:443"
	apiHost = "10.0.5.43:6443"
...
func MemberMetricCollector() {
	...
	createFile("/home/NewForConfig.txt")
	createFile("/home/InClusterConfig.txt")
	createFile("/home/Nodelist.txt")

	for {

		host_config, err := InClusterConfig() // apiserver에 접근해 config파일 읽어오기
		// host_config, _ := rest.InClusterConfig()
		if err != nil {
			writeFile("/home/InClusterConfig.txt", err.Error())
			fmt.Println(err.Error())
			// break
		}
		...
		fmt.Println(host_config.Host)
		fmt.Println(host_config.TLSClientConfig)
		fmt.Println(host_config.BearerToken)
		fmt.Println(host_config.BearerTokenFile)

		host_kubeClient, err := kubernetes.NewForConfig(host_config) // clientSet 생성
		if err != nil {
			writeFile("/home/NewForConfig.txt", err.Error())
			fmt.Println(err.Error())
			// break
		}

		fmt.Println("host_kubeClient: ")
		fmt.Println(host_kubeClient)

		Node_list, err := clusterManager.GetNodeList(host_kubeClient) // Node list 읽어오기
		if err != nil {
			writeFile("/home/Nodelist.txt", err.Error())
			fmt.Println(err.Error())
			for i := 0; i < 1000; i++ {
				fmt.Println(i, " times test")
				time.Sleep(time.Second * 3)
