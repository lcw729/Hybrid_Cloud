...
func ImageLocality(pod *v1.Pod, node *resourceinfo.NodeInfo, totalNumNodes int) int64 {

	if node == nil {
		return -1
	}

	var score = calculatePriority(sumImageScores(node, pod.Spec.Containers, totalNumNodes))
...
func calculatePriority(sumScores int64) int {
	...
	return int(int64(scoretable.MaxNodeScore) * (sumScores - minThreshold) / (maxThreshold - minThreshold))
}
