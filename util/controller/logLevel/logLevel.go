...
func (r *reconciler) Reconcile(req reconcile.Request) (reconcile.Result, error) {
	...
	if req.Namespace == "openmcp" && req.Name == "log-level" {
		...
		if prevLogLevel != logLevel {
			hcplog.Info("LogLevel Changed, Used LogLevel (" + prevLogLevel + " -> " + logLevel + ")")
...
func (r *reconciler) getLogLevel() string {
	...
	if err != nil && errors.IsNotFound(err) {
		hcplog.Info("Not Exist Policy 'log-level', Use Default LogLevel (0)")
		return "0"
	} else if err != nil {
		hcplog.Info("FatalError ! ", err)
	}
	if instance.Spec.PolicyStatus == "Enabled" {
		for _, policy := range instance.Spec.Template.Spec.Policies {
			if policy.Type == "Level" && len(policy.Value) == 1 {
				...
				hcplog.Info("Policy 'log-level' Value must be [-1~9], Use Default LogLevel (0)")
				return "0"
			}
		}
		hcplog.Info("Policy 'log-level' Format Error, Use Default LogLevel (0)")
		return "0"

	} else {
		hcplog.Info("Policy 'log-level' Disabled, Use Default LogLevel (0)")
		return "0"
	}
